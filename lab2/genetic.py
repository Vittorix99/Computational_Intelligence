...
MUTATION_RATE = 0.1
MUTATION_AND_CROSSOVER_RATE = 0.2
NUM_AGENTS = 50
...
def population_init( NUM_AGENTS, good_players, bad_players, num_rows):
    ...
    for _ in range(num_good_players):
        strategy_weights = [np.random.uniform(0.6, 0.9), np.random.uniform(0.4, 0.6), np.random.uniform(0.1, 0.3), np.random.uniform(0, 0.2), np.random.uniform(0, 0.2), np.random.uniform(0, 0.2)]
    ...
    for _ in range(num_bad_players):
        strategy_weights = [np.random.uniform(0.1, 0.2), np.random.uniform(0.2, 0.3), np.random.uniform(0.2, 0.4),np.random.uniform(0.4, 0.7), np.random.uniform(0.4, 0.7), np.random.uniform(0.7, 0.9)]
       # np.random.shuffle(strategy_weights)
        genome_move_weights = [np.random.uniform(0, 1) for _ in range(num_rows)]
        genome = (strategy_weights, genome_move_weights)
        population.append(NimAgent(genome))
...
if __name__ == '__main__':
    ...
    population_init  = population_init(NUM_AGENTS, GOOD_PLAYERS, BAD_PLAYERS, NUM_ROWS)
    generations = 20
    best_fitness = -1 
    nim = Nim(NUM_ROWS)
    best_agent = None
    for generation in range(generations):
        fitness_scores, average_moves = evaluation(population_init, nim)
        print(f"Generation {generation} completed")
        print(f"Best Fitness: {max(fitness_scores)}")
        print(f"Genome: {population_init[fitness_scores.index(max(fitness_scores))]}") 
       ...
       # print(f"Generation {generation} completed")
        ...
        if max_fitness > best_fitness:
            ...
            best_agent = population_init[fitness_scores.index(max_fitness)]
            print(f"Best agent: {population_init[fitness_scores.index(max_fitness)]}")
